...
const loadHistoryQueries = async (): Promise<void> => {
  try {
    const files = await fs.promises.readdir(path.join(__dirname, "./Queries"));
    const sqlFiles = files.filter(file => file.endsWith(".sql"));
    ...
    console.log("✅ Archivos SQL de Historial cargados exitosamente");
  } catch (error) {
    console.error("❌ Error cargando archivos Historial de deudas:", error);
    process.exit(1);
  }
};
