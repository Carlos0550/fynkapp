...
async function retrieveClientData(req, res) {
    ...
    try {
        ...
        const encriptedDni = await encrypt(client_dni);
        const result = await client.query(FRPQueries[0], [encriptedDni, user_id]);
        ...
        let clientData = { client_id, client_name: client_fullname };

        const debtData = await client.query(FRPQueries[1], [client_id, user_id]);
        const clientDebts = debtData.rows[0]?.clientdebts || [];
        const clientDelivers = debtData.rows[0]?.clientdelivers || [];
        ...
        clientData = { ...clientData, total: totalProductsDebt - totalDelivers };
...
async function saveDeliver(req,res) {
    const { "FastRegisterPayment.sql": FRPQueries } = queries;
